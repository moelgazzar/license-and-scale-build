  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <p className="section-eyebrow">Step 2 路 Approval queue</p>
        ...
        <div>
          <h2 className="text-[16px] font-semibold text-[var(--color-ink)]">
            ...
            <span className="ml-2 text-[13px] font-normal text-[var(--color-muted)]">
              {lead.project_type} 路 {lead.city}
            </span>
          </h2>
          <p className="mt-0.5 text-[12.5px] text-[var(--color-muted)]">
            Est. ${lead.est_project_value ? Math.round(Number(lead.est_project_value)).toLocaleString() : '-'} 路
            attempt {lead.attempt_count + 1}/3
          </p>
        </div>
        <div className="text-[11.5px] text-[var(--color-muted)]">
          {draft.model}
          {draft.generation_cost_usd !== null && ` 路 $${Number(draft.generation_cost_usd).toFixed(4)}`}
        </div>
      </header>
